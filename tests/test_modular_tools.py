...
from apex.refactored_tools import (
    FileOperations, GitOperations, CommandOperations, WebOperations,
    create_file_ops, create_git_ops, create_command_ops, create_web_ops
)
...
class TestGitOperations:
    ...
    def test_get_git_log_is_repo(self, tmp_cwd):
        import subprocess
        subprocess.run(["git", "init"], cwd=tmp_cwd, capture_output=True)
        subprocess.run(["git", "config", "user.email", "test@test.com"], cwd=tmp_cwd, capture_output=True)
