...
def test_format_file_rs_error(executor, tmp_path, monkeypatch):
    ...
    f.write_text("fn main(){}")
    monkeypatch.setattr(
        subprocess, "run", lambda *a, **kw: type("R", (), {"returncode": 1, "stderr": "err"})()
    )
    result = executor.execute("format_file", {"path": str(f)})
    assert "ERROR" in result
...
def test_install_package_success(executor, monkeypatch):
    monkeypatch.setattr(
        subprocess, "run", lambda *a, **kw: type("R", (), {"returncode": 0, "stderr": ""})()
    )
...
def test_retry_tool_exception(executor, monkeypatch):
    ...
    result = executor.execute(
        "retry_tool", {"tool": "read_file", "args": {"path": "x"}, "retries": 1}
    )
    assert "ERROR" in result
...
def test_async_executor_parallel():
    ...
    e = AsyncToolExecutor()
    results = asyncio.run(
        e.execute_all_parallel([("get_keybindings", {}), ("set_theme", {"theme": "dark"})])
    )
...
def test_preview_edit_path_not_found(executor):
    result = executor.execute(
        "preview_edit", {"path": "_nonexistent_", "old_string": "a", "new_string": "b"}
    )
    assert "ERROR" in result
...
def test_clipboard_read_xclip_fallback(executor, monkeypatch):
    ...
    monkeypatch.setattr(
        subprocess, "run", lambda *a, **kw: type("R", (), {"returncode": 0, "stdout": "cb_data"})()
    )
...
def test_clipboard_read_xclip_fails(executor, monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "pyperclip", _FakeNoClip)
    monkeypatch.setattr(
        subprocess, "run", lambda *a, **kw: (_ for _ in ()).throw(Exception("no xclip"))
    )
    result = executor.execute("clipboard_read", {})
    assert "ERROR" in result
...
def test_clipboard_read_exception_handler(executor, monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "pyperclip", _FakeNoClip)
    monkeypatch.setattr(
        subprocess, "run", lambda *a, **kw: (_ for _ in ()).throw(OSError("xclip missing"))
    )
    result = executor.execute("clipboard_read", {})
    assert "ERROR" in result
...
def test_git_commit_stderr_error(executor, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "run",
        lambda *a, **kw: type(
            "R", (), {"returncode": 1, "stderr": "nothing to commit", "stdout": ""}
        )(),
    )
    result = executor.execute("git_commit", {"message": "test"})
    assert "ERROR" in result
