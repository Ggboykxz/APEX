"""Tests for refactored session module — no mocks, real filesystem."""
...
class TestUndoManager:
    ...
    def test_undo_redo_cycle(self):
        manager = UndoManager()
        manager.snapshot("action1", {"v": 1})
        manager.snapshot("action2", {"v": 2})
        # Undo twice
        manager.undo()
        manager.undo()
        assert manager.undo_count == 0
...
class TestSessionManager:
    ...
    def test_init_creates_directory(self, tmp_path):
        SessionManager(sessions_dir=tmp_path / "new_sessions")
        assert (tmp_path / "new_sessions").exists()

    def _make_agent(self, model="gpt-4", cwd="/workspace", history=None, usage=None):
        """Create a real agent-like object instead of using MagicMock."""

        class Agent:
            def __init__(self):
                self.model = model
                self.cwd = Path(cwd)
                self.history = history or ["msg1", "msg2"]
                self.usage = usage or {"tokens": 1000}
    ...
    def test_save(self, tmp_path):
        manager = SessionManager(
            sessions_dir=tmp_path, time_factory=lambda: datetime(2024, 1, 1, 12, 0, 0)
        )
        agent = self._make_agent()
        filepath = manager.save(agent, "test_session")
    ...
    def test_save_creates_latest_link(self, tmp_path):
        ...
        agent = self._make_agent()
        manager.save(agent, "test")
        latest_link = tmp_path / "latest_test.json"
        assert latest_link.exists()
        assert latest_link.is_symlink()

    def test_save_replaces_latest_link(self, tmp_path):
        manager = SessionManager(
            sessions_dir=tmp_path, time_factory=lambda: datetime(2024, 1, 1, 12, 0, 0)
        )
        agent = self._make_agent(model="model_v1")
        manager.save(agent, "test")

        agent2 = self._make_agent(model="model_v2")
        manager.save(agent2, "test")
    ...
    def test_load_not_found(self, tmp_path):
        manager = SessionManager(sessions_dir=tmp_path)
        agent = self._make_agent()
        result = manager.load("nonexistent", agent)
    ...
    def test_load_via_symlink(self, tmp_path):
        manager = SessionManager(
            sessions_dir=tmp_path, time_factory=lambda: datetime(2024, 1, 1, 12, 0, 0)
        )
        agent = self._make_agent()
        ...
        agent2 = self._make_agent(model="other_model")
        result = manager.load("test", agent2)
        assert result is True
        assert agent2.model == "gpt-4"

    def test_load_corrupt_file(self, tmp_path):
        session_file = tmp_path / "20240101_120000_bad.json"
        session_file.write_text("not valid json{{{")
        manager = SessionManager(sessions_dir=tmp_path)
        agent = self._make_agent()
        result = manager.load("bad", agent)
        assert result is False

    def test_list_sessions(self, tmp_path):
        (tmp_path / "20240101_test1.json").write_text(
            json.dumps(
                {
                    "name": "session1",
                    "timestamp": "2024-01-01T12:00:00",
        ...
        (tmp_path / "20240102_test2.json").write_text(
            json.dumps(
                {
                    "name": "session2",
    ...
    def test_list_sessions_skips_symlinks(self, tmp_path):
        manager = SessionManager(
            sessions_dir=tmp_path, time_factory=lambda: datetime(2024, 1, 1, 12, 0, 0)
        )
        agent = self._make_agent()
    ...
    def test_list_sessions_skips_corrupt(self, tmp_path):
        (tmp_path / "20240101_good.json").write_text(
            json.dumps({"name": "good", "timestamp": "2024-01-01", "model": "m", "history": []})
        )
        (tmp_path / "20240101_bad.json").write_text("not json")
        manager = SessionManager(sessions_dir=tmp_path)
        sessions = manager.list_sessions()
        assert len(sessions) == 1
    ...
    def test_share(self, tmp_path):
        ...
        agent = self._make_agent()
        share_url = manager.share(agent, "test")
        ...
        share_file = tmp_path / "shared" / f"{share_id}.json"
        assert share_file.exists()

    def test_load_shared_not_found(self, tmp_path):
        manager = SessionManager(sessions_dir=tmp_path)
        agent = self._make_agent()
        result = manager.load_shared("nonexistent_id", agent)
        assert result is False
    ...
    def test_load_shared_success(self, tmp_path):
        ...
        manager = SessionManager(sessions_dir=tmp_path)
        agent = self._make_agent()
        result = manager.load_shared("abc12345", agent)
        assert result is True
        assert agent.model == "gpt-4"

    def test_load_shared_corrupt(self, tmp_path):
        shared_dir = tmp_path / "shared"
        shared_dir.mkdir()
        (shared_dir / "bad12345.json").write_text("not valid json")
        manager = SessionManager(sessions_dir=tmp_path)
        agent = self._make_agent()
        result = manager.load_shared("bad12345", agent)
        assert result is False
