  ...
  return (
    <box id="apex-chat" style={{ flexDirection: "column", flexGrow: 1, backgroundColor: apexTheme.bg }}>
      <box style={{ height: 1, paddingLeft: 1, paddingRight: 1, flexDirection: "row", justifyContent: "space-between", backgroundColor: apexTheme.bgSurface }}>
        ...
        {isGenerating ? (
          <text><span style={{ fg: apexTheme.warning }}>Generating...</span></text>
        ) : (
          <text><span style={{ fg: apexTheme.green }}>Ready</span></text>
        )}
      </box>

      <scrollbox id="apex-chat-messages" style={{ flexGrow: 1, rootOptions: { backgroundColor: apexTheme.bg }, wrapperOptions: { backgroundColor: apexTheme.bg }, viewportOptions: { backgroundColor: apexTheme.bg }, contentOptions: { backgroundColor: apexTheme.bg }, scrollbarOptions: { showArrows: false, trackOptions: { foregroundColor: apexTheme.cyan, backgroundColor: apexTheme.scrollbarTrack } } }}>
        {messages.length === 0 ? (
      ...
      <box style={{ height: 3, flexDirection: "column", border: true, borderColor: apexTheme.borderFocus, borderStyle: "single", backgroundColor: apexTheme.bgSurface }}>
        <input
          id="apex-chat-input"
          ...
          onSubmit={onSubmit}
          focused
          style={{ focusedBackgroundColor: apexTheme.bgSurfaceBright, backgroundColor: apexTheme.bgSurface }}
        />
      </box>
    </box>
  )
